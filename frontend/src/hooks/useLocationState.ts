...
export function useLocationState() {
  ...
  const [locationStatus, setLocationStatus] = useState<string>(() => {
    ...
    return stored ? `Using saved location: ${stored.label}.` : 'Requesting location permission...'
  })
  const [showLocationControls, setShowLocationControls] = useState(false)
  const [locationQuery, setLocationQuery] = useState('')
  const [locationMessage, setLocationMessage] = useState('Enter a city, state, or ZIP code.')
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        applyLocation(favorites[0], 'Location not available. Defaulted to favorite location.')
      },
      { timeout: 10000 },
    )
